...
## 一、产品定位 ✅
<text bgcolor="light-gray">前端已完成 — 官方主页 http://58.87.69.241  </text>**TokenCPS = Claude API / GPT Plus 的 CPS 联盟分销平台**类比淘宝联盟、京东联盟、万单联盟、云瞻开放平台、推客密码。推广员（推手）注册平台，推广 Claude API 套餐和 GPT Plus 账号，按成交获取佣金分成。核心卖点：**零门槛开始赚钱** —— 注册→拿链接→分享→到账。
## 二、用户角色 ✅ 前端已完成 — 推广员注册 http://58.87.69.241/auth/register-promoter | C端注册 http://58.87.69.241/auth/register
### 2.1 推广员（B端）
- 技术圈、SEO从业者、KOL、泛流量主
...
> 说明：58.87.69.241 已规划，但因备案未完成，当前阶段统一以 IP 地址部署和联调。
### 3.2 两个注册入口
- 推广员注册页：tokencps.com/register（教推广流程）✅ http://58.87.69.241/auth/register-promoter
- C端注册页：落地页上的注册（教购买和配置 API）✅ http://58.87.69.241/auth/register
- C端落地页可放"推广赚钱"入口引导去推广员注册
## 四、商品体系 ✅ 前端已完成 — 套餐页 http://58.87.69.241/packages | 产品页 http://58.87.69.241/products
### 4.1 Claude API 套餐
...
### 4.3 活动体系 ✅ 管理后台活动管理已完成 http://58.87.69.241/admin/activities
- 每个推广活动绑定一个货源 + 独立佣金比例
- 管理员后台配置
- selao 和 xinxu 分别对应不同活动页面
- 推广员生成通用链接，用户自己选套餐
## 五、佣金体系 ✅ 前后端已完成 — 佣金引擎v2 lib/services/commission-v2.ts | 佣金明细 http://58.87.69.241/promoter/commission | 佣金API /api/commission/calculate
...
### 5.3 佣金明细流水 ✅ http://58.87.69.241/promoter/commission
每笔变动都有记录，推广员可查看完整流水：
```plaintext
...
### 5.4 推广链接归属 ✅ 推广链接管理 http://58.87.69.241/promoter/links
- **最后点击归属**：用户最后点击的推广链接对应的推广员获得佣金
- 自买允许，不冲突（自买省钱，推广赚钱）
- 自买订单上级也能抽10%
## 六、C端用户体系 ✅ 前端已完成 — 账户页 http://58.87.69.241/account | 购买流程 http://58.87.69.241/packages
### 6.1 账户体系 ✅ http://58.87.69.241/account
- 一个用户一个账户
- 账户下可有多个 API Key
- 充值加到账户余额（显示美元额度，如 "$220 可用"）
- 按实际 API 调用扣费
- 续充也给推广员算佣金
### 6.2 交付方式 ✅ 新手引导含配置教程 http://58.87.69.241/guide
...
- 推广员佣金同步追回
- 上级分成同步追回
## 七、提现体系 ✅ 前端已完成 — 推广员提现 http://58.87.69.241/promoter/withdrawal | 管理后台审核 http://58.87.69.241/admin/withdrawals
...
## 八、裂变机制 ✅ 前端已完成 — 团队管理 http://58.87.69.241/promoter/team | 裂变服务 lib/services/referral.ts
### 8.1 注册送体验额度
...
- 每成功邀请一个推广员注册 → 奖励 ¥5
- 从被邀请人首笔成交佣金里扣
### 8.3 推广海报一键生成 ✅ 推广链接管理页已实现海报弹窗 http://58.87.69.241/promoter/links
- 自动带推广员二维码
...
## 十、推广员信息可见性 ✅ 前端已完成 — 推广用户列表（半脱敏）http://58.87.69.241/promoter/customers
- 推广员可看到自己推广的 C 端用户列表
- **半脱敏**：昵称 + 注册时间 + 充值金额，不显示手机号/邮箱
- 推广员无法获取用户联系方式
## 十一、新手引导 ✅ 前端已完成 — 新手引导页 http://58.87.69.241/guide
### 推广员端 ✅
...
## 十二、落地页设计（两套） ✅ 前端已完成 — 官方首页 http://58.87.69.241
### 技术圈版 ✅ 首页已包含
...
- 极简流程引导
- 支持小额试用（$5 体验额度）
## 十三、通知体系 ✅ 前端已完成 — 通知中心 http://58.87.69.241/notifications | 通知服务 lib/services/notification.ts
- 站内消息通知 ✅
- 实时数据推送（成交通知、佣金到账）✅ Mock 数据
## 十四、数据埋点 ✅ 前端已完成 — 数据分析页 http://58.87.69.241/dashboard/analytics | 管理后台数据概览 http://58.87.69.241/admin | 埋点工具 lib/utils/tracking.ts
### 推广员侧 ✅ 推广员仪表盘 http://58.87.69.241/promoter
- 注册转化率、活跃推广员数、人均推广订单数、推广员留存率
### C端侧 ✅
- 落地页转化率、注册转化率、充值转化率、复充率、客单价
### 财务侧 ✅ 管理后台 http://58.87.69.241/admin
- GMV、佣金支出、平台利润、提现金额
## 十五、法律文档 ✅ 前端已完成
- 用户服务协议（退款规则、免责条款、GPT Plus 无质保）✅ http://58.87.69.241/terms
- 隐私政策（数据收集范围、用途、存储方式）✅ http://58.87.69.241/privacy
- 推广员合作协议（佣金规则、违规处罚、平台权利）✅ http://58.87.69.241/promoter-agreement
## 十六、客服 ⏳ 待配置
- 初期：留微信号/QQ号/企业微信，管理员自行处理
- 前端已预留帮助/反馈入口 http://58.87.69.241/help | http://58.87.69.241/feedback
## 十七、货源管理 ✅ 管理后台已完成 — 活动-货源管理 http://58.87.69.241/admin/activities
