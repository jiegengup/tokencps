...
import { api, mockDailyUsage } from '@/lib/mock-api'
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-card rounded-xl border border-border-light p-8 text-center mb-6">
        ...
        <div className="space-y-4 text-left max-w-sm mx-auto">
          {[
            { n: '1', t: '充值', d: '选择套餐，微信支付宝直付', href: '/recharge' },
            { n: '2', t: '获取 API Key', d: '一键生成，复制即用', href: '/keys' },
            { n: '3', t: '配置到应用', d: '查看接入文档和代码示例', href: '/docs' },
          ].map(s => (
      ...
      <div className="text-center">
        <Link href="/recharge" className="inline-block px-8 py-3 bg-accent text-white rounded-xl text-sm font-medium hover:bg-accent-hover transition-colors">
          立即充值
        </Link>
      </div>
    </div>
...
function ReturningDashboard({ data, keys }: { data: import('@tokencps/shared').ConsumerDashboard; keys: import('@tokencps/shared').APIKeyInfo[] }) {
    ...
    <>
      ...
      {data.remaining < 10 && (
        <div className="bg-danger/10 border border-danger/20 rounded-xl px-5 py-3 mb-6 flex items-center justify-between">
          ...
          <Link href="/recharge" className="text-sm text-danger font-medium hover:underline">去充值</Link>
        </div>
      )}
      ...
      <div className="bg-card rounded-xl border border-border-light p-6 mb-6">
        <div className="flex items-start justify-between mb-4">
          ...
          <Link href="/recharge" className="px-4 py-2 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent-hover transition-colors">
            快捷充值
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="bg-card rounded-xl border border-border-light p-5">
          ...
          <div className="space-y-2">
            <Link href="/keys" className="flex items-center justify-between p-3 rounded-lg hover:bg-hover transition-colors">
              <span className="text-sm text-text">管理 API Key</span>
              <span className="text-xs text-text-tertiary">{data.activeKeys} 个活跃</span>
            </Link>
            <Link href="/usage" className="flex items-center justify-between p-3 rounded-lg hover:bg-hover transition-colors">
            ...
            <Link href="/orders" className="flex items-center justify-between p-3 rounded-lg hover:bg-hover transition-colors">
        ...
        <div className="bg-card rounded-xl border border-border-light p-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link href="/keys" className="text-xs text-accent hover:text-accent-hover">管理</Link>
          </div>
          <div className="space-y-2">
