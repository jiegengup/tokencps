  ...
  return (
    <main className="max-w-5xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {plans.map(p => (
            <div key={p.price} className={`relative bg-card rounded-xl border p-6 text-center transition-shadow hover:shadow-md ${p.hot ? 'border-accent shadow-sm' : 'border-border-light'}`}>
              ...
              <Link href="/auth/register" className="mt-4 block w-full py-2 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent-hover transition-colors">
                立即购买
              </Link>
            </div>
          ))}
      ...
      <section className="text-center pb-16">
        <Link href="/auth/register" className="inline-block px-8 py-3 bg-accent text-white rounded-xl text-base font-medium hover:bg-accent-hover transition-colors">
          注册并开始使用
        </Link>
