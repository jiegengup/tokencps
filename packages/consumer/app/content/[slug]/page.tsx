      ...
      <main className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="mt-12 bg-card rounded-xl border border-border-light p-8 text-center">
          ...
          <div className="flex gap-3 justify-center">
            <Link href="/auth/register" className="px-6 py-2.5 bg-accent text-white rounded-lg text-sm font-medium hover:bg-accent-hover transition-colors">
              免费注册
            </Link>
            <Link href="/recharge" className="px-6 py-2.5 border border-border text-text rounded-lg text-sm font-medium hover:bg-hover transition-colors">
              查看套餐
            </Link>
          </div>
        </div>
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {Object.entries(articles).filter(([k]) => k !== slug).slice(0, 4).map(([k, a]) => (
              <Link key={k} href={`/content/${k}`} className="bg-card rounded-lg border border-border-light p-4 hover:border-border transition-colors">
